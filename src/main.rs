...
use feats::{feat::{Feat, FeatPrereq}, feat_effect::FeatEffect};
...
fn make_feats() -> () {
    ...
    // TODO should add a bonus action ability
    ...
    // Crossbow Expert is uninteresting at the moment

    // TODO should add a reacton ability
    ...
    let _defensive_duelist = Feat::new(
        ...
        Some(feats::feat::FeatPrereq::MinStat { s: vec![(Stat::Dexterity, 13)], stat_logical_or: false }),
        Some(defensive_duelist_effects)
    );

    // Not even sure what to do with Dual Wielder
    ...
    let _grappler = Feat::new(
        ...
        Some(FeatPrereq::MinStat { s: vec![(Stat::Strength, 13)], stat_logical_or: false }), 
